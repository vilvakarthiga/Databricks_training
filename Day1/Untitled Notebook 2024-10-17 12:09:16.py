...
1.rename id col to empid
2.to new col with current timestamp
3.drop is_customer pool
4.concat name and last name
...
df_users.select(col("id").alias("empid")).display()
df_users.withColumn("Current_datetime",current_timestamp()).display()
...
data={"id":1,"name":"a","PN":123}
data={"id":1,"name":"a","PN":[123,888]}
data={"id":1,"name":"a","PN":[123,888]}
